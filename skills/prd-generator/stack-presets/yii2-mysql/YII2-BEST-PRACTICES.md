...
```php
// GOOD — ActiveQuery with scoping
$items = ExampleModel::find()
    ->where(["tenant_id" => $tenantId, "status" => self::STATUS_ACTIVE])
    ->andWhere(["<=", "scheduled_at_utc", time()])
    ->orderBy(["scheduled_at_utc" => SORT_ASC])
    ->all();

// BAD — raw SQL
$items = Yii::$app->db->createCommand("SELECT * FROM items WHERE ...")->queryAll();
```
...
For sensitive fields (API keys, secrets, webhook auth headers):
...
```php
// BAD
BusinessHelper::approve($modelId, $userId);
...
```php
// BAD
$items = ExampleModel::find()->all();
foreach ($items as $item) {
    $owner = Owner::findOne($item->owner_id); // N queries
}

// GOOD
$items = ExampleModel::find()->with(["owner"])->all();
```
